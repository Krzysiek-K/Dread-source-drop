...
#include "demo.h"
 ...
 #define COPYRIGHT_3			"VER ---- (C) KK/ALTAIR 2022"					// <<<<
#endif
#define COPYRIGHT_3			"VER 2A26 (C) KK/ALTAIR 2022"					// <<<<


//#define COPYRIGHT_3			"(C) KK/ALTAIR 2021"
...
{
	return ((MENU_SCREEN_HEIGHT - MENU_FONT_LINESPACING*menu.opt_rowcount)>>1) + index * MENU_FONT_LINESPACING + 10;
}
...
void Menu_ResetBlock(word *screen[], word x1, word y1, word x2, word y2)
{
	...
	BLTAFWM = 0xFFFF;
	BLTALWM = 0xFFFF;
#ifdef MAKE_TEXT_LINE
	BLTCON0 = 0x09FF;		// Force color 31
#else
	BLTCON0 = 0x09F0;
#endif
	BLTCON1 = 0;
...
{
	// fill background
	Menu_ResetBlock(screen_A, 0, 0, 20, MENU_SCREEN_HEIGHT);
	Menu_ResetBlock(screen_B, 0, 0, 20, MENU_SCREEN_HEIGHT);
...
void Menu_PlaceTextImm(word xp, word yp)
{
	Menu_PlaceText(&menu.frame[0], xp, yp);
	Menu_PlaceText(&menu.frame[1], xp, yp);
}


// ================================================================ Console-like printing ================================================================

void Menu_PrintInit(word xp, word yp)
{
	menu.print_xp = menu.print_x0 = xp;
	menu.print_yp = yp;
}
...
void Menu_Print(const char *s)
{
	while(*s)
	{
		...
		{
			s++;
			if( menu.print_yp < 0x7000 )
				menu.print_yp += MENU_FONT_LINESPACING;
			menu.print_xp = menu.print_x0;
		}
		else
		{
			Menu_PrepareTextMask(s);
			if( menu.print_yp + MENU_FONT_HEIGHT <= MENU_SCREEN_HEIGHT )
				Menu_PlaceTextImm(menu.print_xp, menu.print_yp);
			menu.print_xp += menu.textbuff_len;
		}
...
void Menu_PrintHex(dword value)
{
	...
	word numchars = 2;
	*--dst = 0;
	while( numchars || value )
	{
		*--dst = "0123456789ABCDEF"[value&15];
		value >>= 4;
...
{
	io_key_last = 0;
	//while( !io_key_last || (io_key_last&0x80) ) {}
...
int Menu_SelectAndLoadMap(void)
{
	...
	while( Menu_OptionBegin(menu_length) )
	{
		word pos = 0;
		short action = -1;

#if USE_EMBEDDED_MAP_DATA
		if( Menu_Option(pos++, "[EMBEDDED]" ) )
			action = -2;
#endif

		for( int i=0; i<n_files_dir && pos<menu_length; i++ )
			if( Menu_Option(pos++, File_CreateDisplayName(files_dir[i].path)) )
				action = i;
		Menu_OptionEnd();
		...
		if( action == -2 )
		{
			map_loaded = Map_LoadAll(e_globals.skill);
			break;
		}
#endif
		...
		if( action >= 0 )
		{
			Menu_SystemRestore();	// Bring back the system

			map_loaded = (word)Map_FileLoadAll(e_globals.skill, File_CreatePath("maps/", files_dir[action].path));

			Menu_GameRestore();		// Shutdown the system & reinitialize
...
{
	word action = 255;
	//dword ks_checksum = Sys_KSChecksum();
	//ks_checksum = Sys_KSChecksum();
...
#if 0
	//static const byte DEFLATE_STREAM[] ={ 0x78, 0x9C, 0x13, 0x32, 0x09, 0xAB, 0x00, 0x00, 0x02, 0x0C, 0x01, 0x15 };
	static const byte DEFLATE_STREAM[] = { 0x13, 0x32, 0x09, 0xAB, 0x00, 0x00 };
	static dword deflate_buff[5];
	static word inflate_scratch[3000];
	inflate(DEFLATE_STREAM, deflate_buff, inflate_scratch+3000);
#endif

	while( 1 )
	{
		word run = Menu_OptionBegin(SHOW_MEM_STATS ? 8 : 5);
...
#if SHOW_MEM_STATS
		//Menu_Option_V(5, "MEM 1", _itoa(map_memory_buffers[0].size));
		//Menu_Option_V(6, "MEM 2", _itoa(map_memory_buffers[1].size));
		//Menu_Option_V(7, "MEM 3", _itoa(map_memory_buffers[2].size));
		
		Menu_Option_V(5, "MEM 1", _itoa((byte*)map_memory_buffers[0].end - (byte*)map_memory_buffers[0].alloc));
		Menu_Option_V(6, "MEM 2", _itoa((byte*)map_memory_buffers[1].end - (byte*)map_memory_buffers[1].alloc));
		Menu_Option_V(7, "MEM 3", _itoa((byte*)map_memory_buffers[2].end - (byte*)map_memory_buffers[2].alloc));

		//Menu_Option_V(5, "DEF 0", _itoa_hex(deflate_buff[0]));
		//Menu_Option_V(6, "DEF 1", _itoa_hex(deflate_buff[1]));
		//Menu_Option_V(7, "DEF 2", _itoa_hex(deflate_buff[2]));

		//Menu_Option_V(5, "DEF 0", _itoa_hex(((dword*)DEFLATE_STREAM)[0]));
		//Menu_Option_V(6, "DEF 1", _itoa_hex(((dword*)DEFLATE_STREAM)[1]));
		//Menu_Option_V(7, "DEF 2", _itoa_hex(((dword*)DEFLATE_STREAM)[2]));
#endif

		//Menu_Option_V(5, "KS CHK", _itoa_hex(ks_checksum));
		//Menu_Option_V(6, "KS", _itoa_hex(*(dword*)0xF80000));
		//// $07607987	- KS 1.3
		//// $2F157FAC	- KS 2.04
		//// $3AA232CE	- KS 2.05
		////
...
#if ENABLE_MAP_AUTOSTART && ENABLE_MAP_FILE_LOADING				// special case to automatically load maps/start.map
			Menu_SystemRestore();	// Bring back the system
			Map_FileLoadAll(e_globals.skill, "maps/start.map");
			Menu_GameRestore();		// Shutdown the system & reinitialize
			if( !map_error_code )
				break;		// done - run the map

#elif ENABLE_MAP_FILE_LOADING
...
void Menu_MakeText()
{
	...
	while( Menu_OptionBegin(1) )
	{
		word pos = 0;
		Menu_Option(pos++, MAKE_TEXT_LINE);
		Menu_OptionEnd();
	}
}
#endif
