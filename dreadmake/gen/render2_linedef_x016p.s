    ...
    public _Dread_LineCore_x016p
_Dread_LineCore_x016p:

                                                    ;s d01---5-- a01234---       ================================ Sky y1 Upper y2 gap y3 Floor ================================
...
.ret_12:                                            ;R d-----5-7 a--2-----       
    ...
    move.w      d1, rc_ymin-8(a4)                   ;G d---3-5-7 a--2--5--  12   Update rc_ymin with ystart
...
.cmp_10:                                            ;C d-----5-7 a--2--5--       -------- y0 <= ymin < y1 && y1 <= ymax < y2 --------
    ...
    add.w       d3, d3                              ;R d-----5-7 a--2-----   4   top_offset = 4*ymin
    add.w       d3, d3                              ;R d-----5-7 a--2-----   4   
    lea.l       .ret_13(pc), a6                     ;R d-----5-7 a--2-----   8   
    jmp         (a5,d3.w)                           ;R d-----5-7 a--2-----  14   -> .ret_13
.ret_13:                                            ;R d-----5-7 a--2-----       
    move.w      #$1EEA, (a5)                        ;R d-----5-7 a--2-----  12   #$1EEA -> (fill_routine, bottom_offset)
                                                    ;G d-----5-7 a--2--5--       -------- gap] --------
    swap.w      d3                                  ;G d-----5-7 a--2--5--   4   Select ymax
...
.cmp_9:                                             ;C d-----5-7 a--2--5--       -------- ymin < y1 && y2 <= ymax --------
    swap.w      d3                                  ;C d-----5-7 a--2--5--   4   Select ymin
    cmp.w       d0, d3                              ;C d-----5-7 a--2--5--   4   ymin>=y0
    bge         .cmp_14                             ;C d-----5-7 a--2--5--  12   
                                                    ;C d-----5-7 a--2--5--       
                                                    ;C d-----5-7 a--2--5--       -------- ymin < y0 && y2 <= ymax --------
                                                    ;- d-----5-7 a--2--5--       ======== [Sky Upper gap Floor] ========
                                                    ;S d-----5-7 a--2--5--       -------- [Sky --------
    lea.l       _FUNC_SKYCOPY+ENGINE_Y_MAX*2, a5    ;S d-----5-7 a--2-----  12   Sky copy func
    move.l      (a0)+, a2                           ;S d-----5-7 a--------  12   
    add.w       d3, a2                              ;S d-----5-7 a--------   8   sky offset by ymin
    sub.w       d0, d3                              ;S d-----5-7 a--2-----   4   
    add.w       d3, d3                              ;S d-----5-7 a--2-----   4   
    lea.l       .ret_15(pc), a6                     ;S d-----5-7 a--2-----   8   
    jmp         (a5,d3.w)                           ;S d-----5-7 a--2-----  14   -> .ret_15
.ret_15:                                            ;S d-----5-7 a--2--5--       
                                                    ;R d-----5-7 a--2--5--       -------- Upper --------
                                                    ;R d-----5-7 a--2--5--       -- Fill routine --
    move.w      d4, d7                              ;R d-----5-- a--2--5--   4   int lut_offset = (((word)s1)>>3) & ~3;
    lsr.w       #3, d7                              ;R d-----5-- a--2--5--  12   
    ...
    add.w       d7, d7                              ;R d-----5-- a--2-----   4   
    add.w       d7, d7                              ;R d-----5-- a--2-----   4   
    lea.l       .ret_16(pc), a6                     ;R d-----5-- a--2-----   8   
    jmp         (a5,d7.w)                           ;R d-----5-- a--2-----  14   -> .ret_16
.ret_16:                                            ;R d-----5-7 a--2-----       
    move.w      #$1EEA, (a5)                        ;R d-----5-7 a--2-----  12   #$1EEA -> (fill_routine, bottom_offset)
                                                    ;G d-----5-7 a--2--5--       -------- gap --------
    move.w      d1, rc_ymin-8(a4)                   ;G d-----5-7 a--2--5--  12   Update rc_ymin with ystart
    move.l      a7, 0-8(a4)                         ;G d-----5-7 a--2--5--  16   Set future drawing start pointer to current pos
    move.w      d2, rc_ymax-8(a4)                   ;G d-----5-7 a--2--5--  12   Update rc_ymax with yend
    move.w      d2, d7                              ;G d-----5-- a--2--5--   4   Add 2*(yend - ystart) to A7
    sub.w       d1, d7                              ;G d-----5-- a--2--5--   4   
    ...
    jmp         (a3,d3.w)                           ;F d-----5-7 a--2--5--  14   -> .end_loop
                                                    ;C d-----5-7 a--2--5--       
.cmp_14:                                            ;C d-----5-7 a--2--5--       -------- y0 <= ymin < y1 && y2 <= ymax --------
                                                    ;- d-----5-7 a--2--5--       ======== [Upper gap Floor] ========
                                                    ;R d-----5-7 a--2--5--       -------- [Upper --------
                                                    ;R d-----5-7 a--2--5--       -- Fill routine --
    move.w      d4, d7                              ;R d-----5-- a--2--5--   4   int lut_offset = (((word)s1)>>3) & ~3;
    lsr.w       #3, d7                              ;R d-----5-- a--2--5--  12   
...
.cmp_8:                                             ;C d-----5-7 a--2--5--       -------- y1 <= ymin && y1 <= ymax --------
    ...
    bge         .cmp_18                             ;C d-----5-7 a--2--5--  12   
                                                    ;C d---3-5-7 a--2--5--       
                                                    ;C d---3-5-7 a--2--5--       -------- y1 <= ymin < y2 && y1 <= ymax < y2 --------
                                                    ;- d---3-5-7 a--2--5--       ======== [gap] ========
                                                    ;G d---3-5-7 a--2--5--       -------- [gap] --------
    ...
    bra         .end_loop                           ;- d---3-5-7 a--2--5--  12   
                                                    ;C d-----5-7 a--2--5--       
.cmp_18:                                            ;C d-----5-7 a--2--5--       -------- y1 <= ymin && y2 <= ymax --------
    swap.w      d3                                  ;C d-----5-7 a--2--5--   4   Select ymin
    cmp.w       d2, d3                              ;C d-----5-7 a--2--5--   4   ymin>=y2
    bge         .cmp_19                             ;C d-----5-7 a--2--5--  12   
                                                    ;C d-----5-7 a--2--5--       
                                                    ;C d-----5-7 a--2--5--       -------- y1 <= ymin < y2 && y2 <= ymax --------
                                                    ;- d-----5-7 a--2--5--       ======== [gap Floor] ========
                                                    ;G d-----5-7 a--2--5--       -------- [gap --------
    move.w      d2, rc_ymax-8(a4)                   ;G d-----5-7 a--2--5--  12   Update rc_ymax with yend
    sub.w       d2, d3                              ;G d-----5-7 a--2--5--   4   Add 2*(yend - rc_ymin) to A7
    ...
    jmp         (a3,d3.w)                           ;F d-----5-7 a--2--5--  14   -> .end_loop
                                                    ;C d-----5-7 a--2--5--       
.cmp_19:                                            ;C d-----5-7 a--2--5--       -------- y2 <= ymin && y2 <= ymax --------
                                                    ;- d-----5-7 a--2--5--       ======== [Floor] ========
                                                    ;F d-----5-7 a--2--5--       -------- [Floor] --------
