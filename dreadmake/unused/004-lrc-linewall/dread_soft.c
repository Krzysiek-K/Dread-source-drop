...
EngineVertex e_lines[SOFT_MAX_LINES];
...
void Dread_SoftInit(void)
{
	...
	for( int i=0; i<lrc_n_lines; i++ )
	{
		...
		line->v1 = e_vtx + v1;
		line->v2 = e_vtx + v2;
		line->next = e_lines + (i+1)%lrc_n_lines;
		line->other_sector = NULL;
		line->tex_upper = TEXTURE_DATA + lrc_lines_tex[i];
		line->tex_lower = NULL;
		line->xcoord1 = 0;
		line->xcoord2 = lrc_lines_tx2[i];
