...
#if AMIGA_COMM_LEVEL < 2
	//Amiga_Comm_Init_Asm();
	SERPER = 185;		// 19242.46273		372: 9595.437178
	//INTENA = 0x0801;	// disable RBF & TBE
	INTENA = 0x8800;	// enable RBF
	INTENA = 0x0001;	// disable TBE
#endif

}

void Amiga_Comm_LowLevelWrite(byte data)
{
#if AMIGA_COMM_LEVEL == 2
	Amiga_Comm_Out(data);
#elif AMIGA_COMM_LEVEL == 1
	//set_supervisor();
...
{
	Amiga_Comm_LowLevelWrite(0xD5);

	if( comm_tx_checksum==0x5D || comm_tx_checksum==0xD5 )
		comm_tx_checksum ^= 0xAA;

	Amiga_Comm_LowLevelWrite(comm_tx_checksum);

	comm_tx_checksum = 0xA5;

#if AMIGA_COMM_LEVEL == 0
	Comm_Commit();
#endif
}
...
void Comm_Write_Byte(__d0 byte data)
{
	...
	Amiga_Comm_LowLevelWrite(data);
	if( data == 0xD5 )
		Amiga_Comm_LowLevelWrite(0x5D);
}
