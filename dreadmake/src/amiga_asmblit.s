...
; void Blit_DrawImage(__a2 word *screen[], __a3 const word *image, __d2 word xp, __d3 shoort yp);
;
;	d2	- xp
;	d3	- yp
;	d4	* yskip
...
_Blit_DrawImage:
	...
	mulu.w	d5, d6												;
	moveq.l	#0, d4												;															// [d4] = 0			yskip

	cmp.w	#0, d3
	bge.b	.noy1clip
	add.w	d3, d7
	ble		.end
	sub.w	d3, d4												;															// [d4] = dataskip	(in bytes)
	mulu.w	d5, d4
	add.w	d4, d4
	moveq.l	#0, d3
.noy1clip:
...
.waitblit_1:													;
	...
	move.l	#$FFFFFFFF, BLTAWM(a6)								;	BLTAFWM = 0xFFFF;
																;	BLTALWM = 0xFFFF;
	move.w	#0, BLTAMOD(a6)										;	BLTAMOD = 0;				// mask
	move.w	#0, BLTBMOD(a6)										;	BLTBMOD = 0;				// data
...
.noshift:														;	}
	...
	adda.w	#6, a3 												;	word *src = image + 3 + yskip;							// [a3]
	adda.w	d4, a3												;
	move.l	a3, a4												;															// [a4] = src + planestride*5	// in bytes
	add.w	d6, d6												;
	move.w	d6, d0												;
	mulu.w	#5, d0												;
	add.l	d0, a4												;
...
.plane_loop:													;	{
	move.l	a4, BLTAPT(a6)										;		BLTAPT = src + planestride*5;
	move.l	a3, BLTBPT(a6)										;		BLTBPT = src + planestride*plane;
	add.w	d6, a3												;
