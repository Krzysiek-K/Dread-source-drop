...
#define COPYRIGHT_3			"VER 1923 (C) KK/ALTAIR 2021"
...
void Menu_FryHardware(void)
{
	memory_buffer_t *buff = &map_memory_buffers[0];
	...
	int h = buff->size/sizeof(word)/64;
	if( h>1024 ) h = 1024;
	BLTSIZE = COMPUTE_BLTSIZE(64,h);
...
int Menu_Run(void)
{
	...
	if( e_globals.game_state == GAMESTATE_NONE && !ENABLE_MAP_FILE_AUTOSTART )
	{
		Menu_Intro();
	}
	else
