...
void debug_write_hexbuff(word *screenpos, int len, byte *data)
{
	...
	while( len>0 )
	{
		const byte *D1 = NUMFONT + (*data>>4)*6;
		const byte *D2 = NUMFONT + (*data&0x0F)*6;
...
{
	const byte *D1 = NUMFONT + (value/100)*6;
	const byte *D2 = NUMFONT + (value/10%10)*6;
	const byte *D3 = NUMFONT + (value%10)*6;
	word bitmap[6];
