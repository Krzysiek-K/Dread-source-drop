...
int Map_RegionAlloc_Words(mem_region_t *reg, dword size_in_words)
{
	...
	reg->ptr = map_memory_buffer_alloc;
	map_memory_buffer_alloc += size_in_words;
	reg->end = map_memory_buffer_alloc;
...
int Map_RegionAllocAndLoad_Words(mem_region_t *reg, void *file, dword size_in_words)
{
	if( !Map_RegionAllocAndLoad_Words(reg, size_in_words) )
		return 0;

	dword size_pending = size_in_words >> 1;
	byte *read_ptr = (byte*)reg->ptr;
	
	while( size_pending > 0 )
	{
		int size = Sys_Read(file, read_ptr, size_pending);
		if( size<=0 ) return 0;	// bad read or EOF
...
void Map_LoadAndParseFileData(void *file)
{
	...
	while(1)
	{
		...
		if( cmd == MAPFILE_FN_BLOCK_RAW )
		{
			...
			if( !Map_RegionAllocAndLoad_Words(&block->data, file, size) )
				break;
			block->data.ptr = Map_AllocMemory(size);
		}
		else if( cmd == MAPFILE_FN_BLOCK_INIT || cmd == MAPFILE_FN_BLOCK_SETUP )
		{
			if( map_temp_buffer[1] != block->_init_element_size ) break;
